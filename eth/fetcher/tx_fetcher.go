...
func (f *TxFetcher) Enqueue1(peer *eth.Peer, txs []*types.Transaction, direct bool) error {
	...
	for i := 0; i < len(txs); i += 128 {
		...
		//v := f.totalAddTx.Load()
		//*v += uint64(valid)
		//peer.Log().Error("peer addTxs", "time", time.Now().String(), "id", peer.Node().IP(), "totalTxAdd", peer.TotalRecvTx, "allTxAdd", *v, "txs_len", len(batch), "valid", valid)
		//if peer.TotalRecvTx > 1044674407370955161 {
		//	peer.TotalRecvTx = 0
		//}
		//if *v > 1044674407370955161 {
		//	*v = 0
		//	f.totalAddTx.Store(v)
		//}
		if valid > 0 {
